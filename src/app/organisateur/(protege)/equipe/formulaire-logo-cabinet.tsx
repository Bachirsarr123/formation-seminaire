...
import type { EtatUploadLogoCabinet } from './actions';

const ETAT_INITIAL: EtatUploadLogoCabinet = {};

interface Props {
  action: (etat: EtatUploadLogoCabinet, formData: FormData) => Promise<EtatUploadLogoCabinet>;
  aDejaUnLogo: boolean;
}

export function FormulaireLogoCabinet({ action, aDejaUnLogo }: Props) {
  ...
  return (
    <form action={envoyer} className="flex flex-wrap items-center gap-2">
      <label htmlFor={idBase} className="sr-only">
        Logo du cabinet (image, 2 Mo maximum)
      </label>
      <input id={idBase} type="file" name="logo" accept="image/*" required className="max-w-[220px] text-[length:var(--taille-sm)]" />
      <button
        type="submit"
